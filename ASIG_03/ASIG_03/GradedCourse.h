...
public:
	...
	int calcHonorPoints() const;// – devuelve la cantidad de puntos de honor del curso.  Se calcula multiplicando el equivalente numérico de la letra de la nota 
							//(‘A’ = 4, ‘B’ = 3, ‘C’ = 2, ‘D’ = 1, ‘F’ = 0) por la cantidad de créditos.
	bool operator ==(GradedCourse anotherCourse) const;// – devuelve true si dos cursos tienen el mismo código.
	bool operator !=(GradedCourse anotherCourse) const;// – devuelve true si dos cursos no tienen el mismo código.
	bool operator <(GradedCourse anotherCourse) const;// – devuelve true si un curso es menor que el otro
	bool operator <=(GradedCourse anotherCourse) const;// – devuelve true si un curso es menor o igual que el otro.
	bool operator >(GradedCourse anotherCourse) const;// – devuelve true si un curso es mayor que el otro.
	bool operator >=(GradedCourse anotherCourse) const;// – devuelve true si un curso es mayor o igual que el otro.
};
#endif
